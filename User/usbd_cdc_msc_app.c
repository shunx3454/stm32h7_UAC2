...
const struct usb_descriptor cdc_msc_descriptor = {.device_descriptor_callback = device_descriptor_callback,
...
BLOCK_TYPE mass_block[BLOCK_COUNT] __ATTR_SDRAM;
...
void usbd_msc_get_cap(uint8_t busid, uint8_t lun, uint32_t *block_num, uint32_t *block_size) {
  *block_num = g_sdio1.SdCard.BlockNbr; // Pretend having so many buffer,not has actually.
  *block_size = g_sdio1.SdCard.BlockSize;
}
int usbd_msc_sector_read(uint8_t busid, uint8_t lun, uint32_t sector, uint8_t *buffer, uint32_t length) {
  return SD_Read_Disk(buffer, sector, length / 512);
}

int usbd_msc_sector_write(uint8_t busid, uint8_t lun, uint32_t sector, uint8_t *buffer, uint32_t length) {
  return SD_Write_Disk(buffer, sector, length / 512);
}